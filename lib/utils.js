...
function computeDefaults(schema, parentDefaults, definitions) {
  ...
  if (isObject(defaults) && isObject(schema["default"])) {
    ...
    if ("$ref" in schema) {
      // Use referenced schema defaults for this node.
      var _refSchema = findSchemaDefinition(schema.$ref, definitions);

      return computeDefaults(_refSchema, defaults, definitions, formData, includeUndefinedValues);
    }
    ...
    if ("$ref" in schema) {
      // Use referenced schema defaults for this node.
      var _refSchema2 = findSchemaDefinition(schema.$ref, definitions);

      return computeDefaults(_refSchema2, defaults, definitions, formData, includeUndefinedValues);
    }
...
function getMatchingOption(formData, options, definitions) {
  var _loop = function _loop(i) {
    ...
    if (option.properties && formData) {
      var keyList = (0, _keys["default"])(formData);
      var valid = false;
